...
import michmoney
import datetime
from michmoney.api.api_exceptions import InvalidUsage


@michmoney.app.route("/api/admin/add/", methods=["POST"])
...
@michmoney.app.route("/api/admin/remove/", methods=["POST"])
...
@michmoney.app.route("/api/db/dump/", methods=["GET", "POST"])
def db_dump():
    """Dump database in route. Requires admin privileges."""
    api_key = flask.request.headers.get("Authorization")
    michmoney.api.admin.check_admin_priv(api_key)
    conn = michmoney.model.get_db()
    cur = conn.cursor()
...
@michmoney.app.route("/api/db/status/", methods=["POST"])
def db_test():
...
@michmoney.app.route("/api/admin/user-info", methods=["POST"])
def user_info():
    """Get user info."""
    api_key = flask.request.headers.get("Authorization")
    michmoney.api.admin.check_admin_priv(api_key)
...
@michmoney.app.route("/api/admin/update-user/", methods=["POST"])
def update_user():
    ...
    if "email" in input_json:
        ...
        if michmoney.api.auth.check_email_exists(input_json["email"]):
...
@michmoney.app.route("/api/admin/delete-user/", methods=["POST"])
def delete_user():
    """Delete a user."""
    api_key = flask.request.headers.get("Authorization")
    check_admin_priv(api_key)
    ...
    if not isinstance(input_json["username"], str):
        raise InvalidUsage("Parse Error. Username must be a string.")
    username = input_json["username"]
    if not michmoney.api.auth.check_user_exists(username):
